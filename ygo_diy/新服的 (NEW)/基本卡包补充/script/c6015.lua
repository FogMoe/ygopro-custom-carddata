...
function c6015.initial_effect(c)
	...
	e3:SetCode(EEVENT_TO_GRAVE)
	e3:SetRange(LOCATION_SZONE)
	e3:SetProperty(EFFECT_FLAG_CARD_TARGET+EFFECT_FLAG_DAMAGE_STEP)
	e3:SetCountLimit(1)
	e3:SetCondition(c6015.erascon)
	e3:SetTarget(c6015.atktg2)
	e3:SetOperation(c6015.atkop2)
	c:RegisterEffect(e3)
end
function c6015.erascon(e)
	return e:GetHandler():IsReason(REASON_DESTROY)
end
function c6015.filter2(c,e,tp)
	return c:IsReason(REASON_BATTLE+REASON_EFFECT) and c:IsType(TYPE_MONSTER)
		and c:IsPreviousLocation(LOCATION_MZONE) and c:IsLocation(LOCATION_GRAVE) and c:IsCanBeEffectTarget(e)
end
