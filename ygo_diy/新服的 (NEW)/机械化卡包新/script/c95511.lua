...
function cm.activate(e)
	...
	if tc and tc:IsRelateToEffect(e) and tc:IsDefensePos() then
		if Duel.SendtoHand(tc,nil,REASON_EFFECT) and Duel.SelectEffectYesNo(tp,aux.Stringid(m,2)) and Duel.IsExistingMatchingCard(cm.jixiehua,tp,LOCATION_HAND,0,1,nil) then
			Duel.BreakEffect()
